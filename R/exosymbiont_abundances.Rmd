...
```{r}
summary(emmeans::emmeans(m.dflav.1, pairwise ~ nutrients), type = "response")$emmeans 
```
...
```{r}
emm.dflav <- as_tibble(summary(emmeans::emmeans(m.dflav.1, pairwise ~ nutrients), type = "response")$emmeans) 
```
...
Check magnitude of effect

```{r}
summary(emmeans::emmeans(m.gmono.1, pairwise ~ crab), type = "response")$emmeans
```
...
```{r}
emm.gmono <- as_tibble(summary(emmeans::emmeans(m.gmono.1, pairwise ~ crab), type = "response")$emmeans) 
```
